from src.Common.Library import exists, exist, set_exists, operate, create_square_names, true_list
DEFAULT_MOVE: list = [(0, 0), (0, 0)]
...
class ChessLogic:
    def __init__(self, color: str = '?'):
        ...
        if color == 'b':
            ...
            self.castling = BlackCastling()
        else:
            self.castling: Castling = Castling()
        self.last_move: list = DEFAULT_MOVE
        ...
        for r in range(len(self.chessboard)):
            for c in range(len(self.chessboard)):
                if self.chessboard[r][c][0] == color:
                    _ms: set = Position(self.chessboard, r, c, self.last_move).move_set()
                    _ms = self.prevent_the_check(r, c, _ms)
                    ms.update(_ms)
        return len(ms) == 0
    ...
    def prevent_the_check(self, x, y, _ms: set):
        ms: set = set()
        for p in _ms:
    ...
    def castling_procedure(self, _set: set, x: int, y: int) -> set:
        self.castling.update(self.chessboard, self.move_set_for)
        ms: set = _set
        p: list = [(7, 5), (7, 1), (0, 5), (0, 1)] if self.color == 'b' else [(7, 2), (7, 6), (0, 2), (0, 6)]
    ...
    def move_set(self, x: int, y: int) -> set:
        ms: set = Position(self.chessboard, x, y, self.last_move).move_set()
        ms = self.castling_procedure(ms, x, y)
        ms = self.prevent_the_check(x, y, ms)
        return ms
...
class Position:
    ...
    def rook(self) -> set:
        ms: set = set()
        ranges: list = [(self.x - 1, -1, -1), (self.x + 1, len(self.chessboard), 1),
                        (self.y + 1, len(self.chessboard), 1), (self.y - 1, -1, -1)]
        for index, _range in enumerate(ranges):
            for c in range(_range[0], _range[1], _range[2]):
                xy = (c, self.y) if index < 2 else (self.x, c)
                if self.chessboard[xy[0]][xy[1]][0] == self.color:
                    break
                ms.add((xy[0], xy[1]))
                if self.chessboard[xy[0]][xy[1]] != '--':
                    break
        return ms

    def knight(self) -> set:
        ms: set = set()
        points: list = [(self.x - 2, self.y - 1), (self.x - 2, self.y + 1), (self.x + 2, self.y - 1),
    ...
    def king(self) -> set:
        ms: set = set()
        points: list = [(self.x, self.y + 1), (self.x, self.y - 1), (self.x + 1, self.y),
...
class BlackCastling:
    ...
    def safe_position(self, bms: set, wms: set) -> None:
        self.long_w[2] = False if (7, 1) in bms or (7, 2) in bms else True
        self.short_w[2] = False if (7, 4) in bms or (7, 5) in bms else True
        self.long_b[2] = False if (0, 4) in wms or (0, 5) in wms else True
        self.short_b[2] = False if (0, 1) in wms or (0, 2) in wms else True

    def clear_way(self, chessboard: list) -> None:
        self.long_w[1] = False if chessboard[7][4] != '--' or chessboard[7][5] != '--' or chessboard[7][6] != '--' else True
        self.short_w[1] = False if chessboard[7][1] != '--' or chessboard[7][2] != '--' else True
        self.long_b[1] = False if chessboard[0][4] != '--' or chessboard[0][5] != '--' or chessboard[0][6] != '--' else True
        self.short_b[1] = False if chessboard[0][1] != '--' or chessboard[0][2] != '--' else True

    def not_moved_pieces(self, chessboard: list) -> None:
        if chessboard[0][3] != 'bK':
            self.long_b[0] = False
        ...
        if chessboard[7][7] != 'wR':
            ...
            return None
        if chessboard[7][3] != 'wK':
            self.long_w[0] = False
            self.short_w[0] = False

    def update(self, chessboard: list, move_set_for) -> None:
        self.clear_way(chessboard)
        self.safe_position(move_set_for('b'), move_set_for('w'))
