...
from src.ClientMultipalyer.ChessLogic import ChessLogic, DEFAULT_MOVE
from src.Common.Library import exist, image_label, sound, operate, convert, image
from src.Common.Constants import S
...
class ChessGUI(QWidget):
    def __init__(self, parent: QWidget, color: str = '?'):
        ...
        self.audio_player: QMediaPlayer = QMediaPlayer()
        self.sound: str = ''
    ...
    def manual_is_castling(self) -> bool:
        rp, ep = [], []
        if self.chess.chessboard[x1][y1] == 'wK':
            if x1 == 7 and y1 == 3 and x2 == 7 and y2 == 1:
                rp, ep = [7, 0], [7, 2]
            elif x1 == 7 and y1 == 3 and x2 == 7 and y2 == 5:
                rp, ep = [7, 7], [7, 4]
        elif self.chess.chessboard[x1][y1] == 'bK':
            if x1 == 0 and y1 == 3 and x2 == 0 and y2 == 1:
                rp, ep = [0, 0], [0, 2]
            elif x1 == 0 and y1 == 3 and x2 == 0 and y2 == 5:
                rp, ep = [0, 7], [0, 4]
        if len(rp) == len(ep) == 2:
